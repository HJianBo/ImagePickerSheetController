...
open class ImagePickerAction {
    ...
    public convenience init(title: String, secondaryTitle: String? = nil, style: ImagePickerActionStyle = .default, handler: @escaping Handler, secondaryHandler: SecondaryHandler? = nil) {
        let secondaryTitle2: Title? = secondaryTitle.map { string in { _ in string }}
        self.init(title: title, secondaryTitle: secondaryTitle2, style: style, handler: handler, secondaryHandler: secondaryHandler)
    }
